...
export const AnalysisResults: React.FC = () => {
    ...
    // 2. Checklist Toggle Handle
    const toggleChecklist = (item: string) => {
        if (!result) return;
        
        const completed = result.completedChecklistItems || [];
        ...
        <div className="max-w-6xl mx-auto space-y-8 pb-10">
            ...
            {weakSkills.length > 0 && (
                <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-6 flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <button onClick={() => navigate('practice')} className="px-6 py-2 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 transition shadow-sm">
                        Start Day 1 Plan
                    </button>
                </div>
            )}
                                    ...
                                    <div className="space-y-2">
                                        {items.map((item, i) => (
                                            <label key={i} className="flex items-start gap-2 cursor-pointer group">
                                                <input 
                                                    type="checkbox" 
                                                    checked={(result.completedChecklistItems || []).includes(item)}
                                                    onChange={() => toggleChecklist(item)}
                                                    className="mt-1 h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" 
                                                />
                                                <span className={cn(
                                                    "text-sm transition-colors",
                                                    (result.completedChecklistItems || []).includes(item) ? "text-gray-400 line-through" : "text-gray-600 group-hover:text-gray-900"
                                                )}>{item}</span>
                                            </label>
                                        ))}
                                    </div>
