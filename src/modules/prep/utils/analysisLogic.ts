...
export const generateAnalysis = (company: string, role: string, jdText: string, userSkills: string[] = []) => {
    ...
    // Initialize map with comparison logic
    const skillConfidenceMap: Record<string, 'know' | 'practice'> = {};
    Object.values(extractedSkills).flat().forEach(skill => {
        // If skill exists in user's resume, mark as 'know', else 'practice'
        skillConfidenceMap[skill] = userSkillsSet.has(skill.toLowerCase()) ? 'know' : 'practice';
    });
