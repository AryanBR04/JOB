...
export const ResumePreview: React.FC = () => {
    ...
    const hasData = personalInfo.fullName || 
                    ...
                    experience.length > 0 || 
                    projects.length > 0 || 
                    (typeof skills === 'string' ? skills.length > 0 : (skills.technical.length > 0 || skills.soft.length > 0 || skills.tools.length > 0));
    ...
    const ModernSidebar = () => (
        <aside className="resume-sidebar bg-[var(--accent-color)] text-white p-6 flex flex-col gap-6 h-full min-h-[297mm]" style={accentStyle}>
    ...
    const MainContent = () => (
        <div className={`flex-1 ${selectedTemplate === 'modern' ? 'p-8 pt-10' : ''}`}>
            ...
            {selectedTemplate !== 'modern' && (
                <header id="resume-header" className={`text-center mb-8 ${selectedTemplate === 'classic' ? 'border-b-2 border-[var(--accent-color)] pb-6' : ''}`} style={accentStyle}>
    ...
    // Root Container
    return (
        <div className="resume-preview-wrapper overflow-visible">
            <div
                className={`bg-white shadow-xl min-h-[297mm] w-full max-w-[210mm] mx-auto text-gray-800 print:shadow-none print:m-0 print:p-0 ${selectedTemplate === 'classic' ? 'font-serif' : 'font-sans'}`}
                id="resume-preview"
            >
                {selectedTemplate === 'modern' ? (
